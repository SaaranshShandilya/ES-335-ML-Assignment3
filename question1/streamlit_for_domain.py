...
model = torch.load('code_predictor.pth', weights_only=False)

with open('exported_variables_for_domain.pkl', 'rb') as f:
    stoi = pickle.load(f)
    itos = pickle.load(f)
...
def generate_tokens(model, itos, stoi,existing_string,approach, block_size=5, max_len=20,temperature = 1.0):
    model.eval()
    context = []
...
if st.button("Predict"):
    sentence = generate_tokens(model, itos, stoi, context,approach, block_size,max_k_words, temperature )
